...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group relative bg-white/65 backdrop-blur-md rounded-[1.1rem] sm:rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-white/40 flex flex-col h-full">
      ...
      <Link to={`/product/${product.id}`} className="contents">
        <div className="relative aspect-square w-full shrink-0 overflow-hidden bg-rose-50 rounded-t-[1.1rem] sm:rounded-[1.8rem] m-0 sm:m-2 border-b border-rose-100/50">
          <img 
            src={imageUrl} 
            alt={product.name}
          ...
          <div className="absolute top-2 right-2 sm:top-4 sm:right-4 hidden sm:flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 z-10">
            <button 
              onClick={(e) => {
                 e.preventDefault();
        ...
        <div className="p-2.5 sm:p-5 sm:pt-2 flex flex-col flex-grow min-w-0 justify-center">
          <div className="mb-2 sm:mb-4">
            <div className="flex justify-between items-start mb-0.5 sm:mb-1">
              <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.14em] sm:tracking-[0.2em] text-rose-400 line-clamp-1">
                {displayCategory}
              </span>
              <ArrowUpRight className="w-3 h-3 sm:w-4 sm:h-4 text-slate-300 group-hover:text-rose-400 transition-colors shrink-0" />
            </div>
            
            <h3 className="text-[11px] sm:text-lg md:text-xl font-bold text-slate-800 leading-tight group-hover:text-rose-500 transition-colors mb-1 sm:mb-2 line-clamp-2">
              {product.name}
            </h3>

            <p className="text-slate-500 text-[9px] sm:text-xs italic line-clamp-1 sm:line-clamp-2 leading-relaxed min-h-0 sm:min-h-[32px]">
              {product.description || "Handcrafted with love and care for your special moments."}
            </p>
          </div>

          <div className="mt-auto flex items-center justify-between border-t border-rose-50 pt-2 sm:pt-4 gap-1.5 sm:gap-2">
            <div className="min-w-0">
              <span className="text-[8px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Price</span>
              <p className="text-[12px] sm:text-lg md:text-xl font-black text-slate-900 truncate">Rs. {product.price}</p>
            </div>
            ...
            <button 
              ...
              className="min-h-[36px] min-w-[36px] sm:min-h-[44px] sm:min-w-[44px] p-2 sm:p-3 bg-slate-900 text-white rounded-xl sm:rounded-2xl hover:bg-rose-500 transition-all active:scale-95 shadow-md shrink-0"
            >
